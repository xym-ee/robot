...
typedef actionlib::SimpleActionServer<action_learn::sumAction> Server;


void cb(const action_learn::sumActionConstPtr &goal, Server* server)
{
    //获取目标值
    int num = goal->num;
    ROS_INFO("目标值:%d",num);
    //累加并响应连续反馈
    int result = 0;
    action_learn::sumFeedback feedback;
...
int main(int argc, char *argv[])
{
    ...
    */
    ...
    Server server(nh,"sumint",boost::bind(&cb,_1,&server),false);
    server.start();
    // 5.处理请求,产生反馈与响应;
